...
export function TechCards({techstackPromise}) {
    const techStacks = use(techstackPromise);
    return <>
        <div className="mb-6 container mx-auto p-3">
        <div className="">
        <h1 className="text-[40px] font-extrabold leading-none">Explore the <span className="text-[#DB2777]">Technologies</span></h1>
        <p className="text-[20px] text-[#475569] mt-2">Pick one technology per category to build your ideal stack.</p>
            </div>
            <div className="grid grid-cols-12">
            <div className="grid grid-cols-3 col-span-9 gap-5 items-stretch">{
                techStacks.map(techStack=> <TechCard techStack={techStack} key={techStack.id}></TechCard>)
            }
            </div>
